...
@Component({
  ...
  template: `
      <button class="text-card-foreground duration-300" (click)="toggle()">
...
export class DarkModeToggleComponent  {
  ...
  public darkMode = signal<keyof typeof DarkMode>(DarkMode.dark)
  public DarkMode: typeof DarkMode= DarkMode;
  ...
  constructor() {
    effect(() => {
      if (this.darkMode() === DarkMode.dark) {
        this.document.body.classList.add(DarkMode.dark);
      } else {
        this.document.body.classList.remove(DarkMode.dark);
      }
    });
  }
   ...
   */
  public toggle(): void {
    this.darkMode.set(this.darkMode() === DarkMode.dark ? DarkMode.light : DarkMode.dark);
  }
}
